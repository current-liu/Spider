...
class Output(object):
    def __init__(self):
        self.conn = connector.connect(host="192.168.1.154", user="root", password="root", database="hkstar",
                                      charset="big5", buffered=True)

    def insertTitle(self, title):
        insert_sql = "insert into title values('" + title['titleId'] + "','" + title['title'] + "','" + title[
            'author'] + "','" + title['publishTime'] + "')"
        print(insert_sql)
