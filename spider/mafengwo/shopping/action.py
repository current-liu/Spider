...
def get_shopping_shop():
    ...
    for shop in shops:
        ...
        try:
            shop_name, loc, tel, score, star, review_num = h_parser.parser_shopping_shops(doc)
            msg2 = "shopId:'%s' parser complete，inserting into database" % shop_id
            print msg2
            dao.update_shopping_shop(shop_name, loc, tel, score, star, review_num, shop_id)
        except:
...
def get_shopping_review_on_page(shop_id, page_num):
    ...
    while (flag):
        index += 1
        url = "http://www.mafengwo.cn/gonglve/ajax.php?act=get_poi_comments&poi_id=" + str(
            shop_id) + "&type=0&order=0&category=&page=" + str(index) + "&ts=1503368898773"
        if index == page_num:
            flag = False
        time.sleep(random.uniform(2, 3))
        doc, msg = html_download.downloadPage_without_proxy(url, fo_log)
